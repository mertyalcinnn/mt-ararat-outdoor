...
export default function InstagramFeed({ 
  ...
  useEffect(() => {
    if (!initialPosts || initialPosts.length === 0) {
      // Eğer server tarafından posts gelmezse, client tarafında çekmeyi dene
      const fetchPosts = async () => {
        setLoading(true);
        try {
          const response = await fetch(`/api/instagram?username=${instagramUsername}`);
          if (response.ok) {
  ...
  }, [initialPosts, instagramUsername]);
