...
export default async function ActivitiesPage({ params }: { params: { lang: Locale } }) {
  console.log(`[${new Date().toISOString()}] [${params.lang}] Aktiviteler sayfası render ediliyor...`);
  
  // Dil sözlüğünü al
  const dict = getDictionary(params.lang);
  
  // Önbelleği atlayarak aktiviteleri al
  console.log(`[${new Date().toISOString()}] [${params.lang}] Aktiviteler sayfası: Aktiviteler alınıyor...`);
  const activities = await getAllActivities();
  console.log(`[${new Date().toISOString()}] [${params.lang}] Aktiviteler sayfası: ${activities?.length || 0} aktivite alındı`);
          ...
          >
            <div className="relative h-56 w-full bg-gray-200">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                <span className={`inline-block px-3 py-1 text-xs rounded-full text-white ${
